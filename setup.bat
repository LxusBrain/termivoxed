...
REM Optional: Local TTS (Coqui TTS)
echo.
echo ==========================================
...
if /i "%install_coqui%"=="y" (
    ...
    if %ERRORLEVEL% equ 0 (
        ...
        REM Install faster-whisper for word-level subtitle timing
        echo.
        ...
        if %ERRORLEVEL% equ 0 (
            ...
            echo WARNING: faster-whisper installation failed - subtitles will use estimated timing
            echo   You can install it later: pip install faster-whisper
        )
    ) else (
        echo WARNING: Coqui TTS installation failed - continuing without local TTS
