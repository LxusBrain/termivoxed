...
6. [Multi-Video Combination (TermiVoxed)](#multi-video-combination)
7. [Quality Control & Individual Control](#quality-control--individual-control)
...
### Purpose: Combine multiple processed videos into a single seamless output (TermiVoxed feature)

**Code Block (from core/video_combiner.py):**
...
def combine_project_videos(
    ...
    """
    Combine multiple processed videos from a project in order

    Args:
        videos: List of Video instances (with metadata)
        processed_video_paths: List of paths to processed videos (in order)
        output_path: Where to save combined video
        temp_dir: Temporary directory for intermediate files
        quality: Quality preset
        force_export: Force combination even if incompatible

    Returns:
        True if successful
    """
    try:
        # Check if all videos are compatible for combination
        is_compatible, first_video = VideoCombiner.check_videos_compatible(processed_video_paths)

        if is_compatible:
            # Simple concat - all videos have same specs (fast)
            logger.info("✓ All videos compatible - using fast concat")
            return VideoCombiner.combine_videos_simple(processed_video_paths, output_path)
        else:
            # Complex combination - normalize to common specs
            logger.warning("⚠ Videos have different specs - normalizing...")

            if not force_export:
                logger.error("Cannot combine incompatible videos without force_export=True")
                return False

            return VideoCombiner.combine_videos_complex(
...
**FFmpeg Command (Simple Concat - Compatible Videos):**

```bash
# Step 1: Create concat file (temp/concat_list.txt)
file '/abs/path/to/video1.mp4'
file '/abs/path/to/video2.mp4'
file '/abs/path/to/video3.mp4'

# Step 2: Concatenate using concat demuxer (fast, no re-encoding)
ffmpeg -f concat -safe 0 \
  -i concat_list.txt \
  -c copy \
  -y output_combined.mp4
```

**FFmpeg Command (Complex Concat - Incompatible Videos):**

```bash
# Normalize each video to common specs, then concat
# Step 1: Normalize video1 (scale + fps + audio)
ffmpeg -i video1.mp4 \
  ...
  -y video1_normalized.mp4

# Step 2: Normalize video2 (same process)
ffmpeg -i video2.mp4 \
  -vf "scale=1920:1080:force_original_aspect_ratio=decrease,\
       pad=1920:1080:(ow-iw)/2:(oh-ih)/2,\
       fps=30,\
...
```python
def check_videos_compatible(video_paths: List[str]) -> Tuple[bool, Optional[dict]]:
    """
    Check if videos have compatible specs for simple concatenation

    Returns:
        (is_compatible, first_video_info)
    """
    if not video_paths:
        return False, None

    # Get specs for first video
    first_video = FFmpegUtils.get_video_info(video_paths[0])
    if not first_video:
        return False, None

    # Compare all other videos to first
    for video_path in video_paths[1:]:
        video_info = FFmpegUtils.get_video_info(video_path)

        if not video_info:
            return False, first_video

        # Check resolution
        if (video_info['width'] != first_video['width'] or
            video_info['height'] != first_video['height']):
            logger.warning(f"Resolution mismatch: {video_path}")
            return False, first_video

        # Check FPS
        if abs(video_info['fps'] - first_video['fps']) > 0.1:
            logger.warning(f"FPS mismatch: {video_path}")
            return False, first_video

        # Check codec (recommended but not required)
        if video_info['codec'] != first_video['codec']:
            logger.warning(f"Codec difference: {video_path}")
            # Not a hard failure

    return True, first_video
```

**Key Points:**

- **Compatibility Check:** Compares resolution, FPS, codec across all videos
- **Simple Concat:** Uses `-c copy` for identical specs (fast, lossless)
- **Complex Concat:** Normalizes videos to common specs before combining
- **Aspect Ratio Preservation:** Uses `force_original_aspect_ratio=decrease` + `pad` to avoid distortion
- **Audio Normalization:** All videos normalized to 44100Hz stereo AAC
- **Force Export:** Allows combining incompatible videos with warnings
- **Individual Control:** Each video processed separately before combination

**Orientation Handling:**

TermiVoxed detects video orientation and validates compatibility:

- **Landscape (🖥):** aspect_ratio > 1.1 (e.g., 16:9, 1920x1080)
- **Portrait (📱):** aspect_ratio < 0.9 (e.g., 9:16, 1080x1920)
- **Square (⬜):** 0.9 ≤ aspect_ratio ≤ 1.1 (e.g., 1:1, 1080x1080)

Warning issued if combining videos with different orientations.
