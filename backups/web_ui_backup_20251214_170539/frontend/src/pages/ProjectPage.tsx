...
import { projectsApi, segmentsApi, exportApi, llmApi, videosApi, ttsApi, WS_BASE_URL, type BGMTrack } from '../api/client'
...
function ExportModal({
  ...
  // Check TTS connectivity when modal opens
  useEffect(() => {
    if (isOpen && !isExporting) {
      setTtsStatus(prev => ({ ...prev, checking: true, error: null }))
                ...
                <div className="flex items-center gap-2">
                  {ttsStatus.checking ? (
                    <>
                      ...
                      <span className="text-sm text-text-muted">Checking TTS service...</span>
                    </>
                  ) : ttsStatus.connected ? (
                    <>
                      <Wifi className="w-4 h-4 text-green-400" />
                      <span className="text-sm text-green-400">
                        TTS service connected {ttsStatus.mode === 'proxy' ? '(via proxy)' : ''}
                      </span>
                    </>
                  ) : (
                    <>
                      ...
                      <span className="text-sm text-red-400">
                        {ttsStatus.error || 'TTS service unavailable'}
                      </span>
                    </>
                  )}
                </div>
                {!ttsStatus.checking && !ttsStatus.connected && (
                  <button
...
type AIProviderType = 'ollama' | 'openai' | 'anthropic' | 'custom'
...
function AddSegmentModal({
  ...
  // Set default model when provider or models change
  useEffect(() => {
    if (aiProvider === 'ollama' && ollamaModels.length > 0 && !aiModel) {
      setAiModel(ollamaModels[0].name)
    } else if (aiProvider === 'openai' && !aiModel) {
      setAiModel(AI_PROVIDER_MODELS.openai[0])
    } else if (aiProvider === 'anthropic' && !aiModel) {
      setAiModel(AI_PROVIDER_MODELS.anthropic[0])
    }
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setEndTime(Math.min(startTime + 10, videoDuration))
      setText('')
      setDescription('')
  ...
  const generateScript = async () => {
    ...
    if (aiProvider !== 'ollama' && !aiApiKey.trim()) {
      toast.error(`Please enter your ${aiProvider === 'openai' ? 'OpenAI' : aiProvider === 'anthropic' ? 'Anthropic' : 'API'} key`)
      return
    }
    ...
    try {
      ...
      if (aiProvider === 'custom') {
        providerConfig.endpoint = customEndpoint
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        className="relative bg-terminal-surface border border-terminal-border rounded-lg w-full max-w-lg max-h-[90vh] flex flex-col"
      >
        {/* Fixed header */}
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                value={startTime}
                readOnly
                className="input-base w-full font-mono bg-terminal-bg"
              />
            </div>
            <div>
          ...
          {/* Toggle between manual text and AI generation */}
          <div className="flex items-center gap-2 p-2 rounded bg-terminal-bg border border-terminal-border">
            <button
              onClick={() => setShowAIGenerate(false)}
              className={clsx(
                'flex-1 py-1.5 px-3 rounded text-sm transition-colors',
                !showAIGenerate
            ...
            <button
              ...
              className={clsx(
                'flex-1 py-1.5 px-3 rounded text-sm transition-colors flex items-center justify-center gap-1',
                showAIGenerate
                  ? 'bg-accent-red text-white'
                  : 'text-text-muted hover:text-text-primary'
              )}
          ...
          {showAIGenerate ? (
            <div className="space-y-3">
              ...
              <div className="p-3 rounded bg-terminal-bg border border-terminal-border space-y-3">
                ...
                {/* Provider Tabs */}
                <div className="flex gap-1 p-1 rounded bg-terminal-elevated">
                  {(['ollama', 'openai', 'anthropic', 'custom'] as const).map((p) => (
                    <button
                      key={p}
                      onClick={() => setAiProvider(p)}
                      className={clsx(
                        'flex-1 py-1.5 px-2 rounded text-xs transition-colors capitalize',
                        aiProvider === p
                          ? 'bg-accent-red text-white'
                          : 'text-text-muted hover:text-text-primary'
                      )}
                ...
                {aiProvider !== 'ollama' && (
                  <div>
                    <label className="text-xs text-text-muted mb-1 block">
                      {aiProvider === 'openai' ? 'OpenAI' : aiProvider === 'anthropic' ? 'Anthropic' : 'API'} Key
                    </label>
                    <input
                      type="password"
                      value={aiApiKey}
                      onChange={(e) => setAiApiKey(e.target.value)}
                      placeholder={`Enter ${aiProvider === 'openai' ? 'OpenAI' : aiProvider === 'anthropic' ? 'Anthropic' : 'API'} key`}
                      className="input-base w-full text-sm"
                    />
                  </div>
                )}
              ...
              <div>
                <label className="section-header">Describe this segment</label>
                <textarea
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  rows={3}
                  className="input-base w-full resize-none"
                  placeholder="Describe what happens in this segment... (e.g., 'Introduction scene showing the product from multiple angles')"
                />
              </div>
              <button
              ...
              {text && (
                <div>
                  <label className="section-header">Generated Script</label>
                  <textarea
                    value={text}
                  ...
                  />
                  <p className="text-xs text-text-muted mt-1">
                    You can edit the generated script before creating
                  </p>
                </div>
              )}
            </div>
            ...
            <div>
              <label className="section-header">
                Script Text <span className="text-text-muted font-normal">(optional)</span>
              </label>
              <textarea
                value={text}
                onChange={(e) => setText(e.target.value)}
...
export default function ProjectPage() {
  ...
  const playheadVideoId = useMemo(() => {
    if (!project || project.videos.length <= 1) return project?.active_video_id || null

    const sortedVideos = [...project.videos].sort((a, b) => a.order - b.order)
  ...
  const effectiveActiveVideo = useMemo(() => {
    ...
    return project.videos.find(v => v.id === playheadVideoId) || activeVideo || project.videos[0]
  }, [project, playheadVideoId, activeVideo])
  ...
  const videoOffset = useMemo(() => {
    if (!project || !effectiveActiveVideo || project.videos.length <= 1) return 0
    ...
    return offset
  }, [project, effectiveActiveVideo])

  // Calculate total timeline duration for multi-video playback
  const totalTimelineDuration = useMemo(() => {
    if (!project || project.videos.length === 0) return 0

    const hasTimelinePositions = project.videos.some(v => v.timeline_start !== null && v.timeline_start !== undefined)

    if (hasTimelinePositions) {
      // Find the maximum timeline_end
      return Math.max(...project.videos.map(v => {
      ...
      return project.videos.reduce((sum, v) => sum + (v.duration || 0), 0)
    }
  }, [project])
  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col p-4 overflow-hidden">
          {project.videos.length > 1 ? (
            ...
            <VideoPlayer videoUrl={videoUrl} duration={videoDuration} videoOffset={videoOffset} />
          )}
          ...
          <div className="mt-4">
            <Timeline
              ...
              onAddSegment={(time, videoId) => {
                ...
                setAddSegmentVideoId(videoId || effectiveActiveVideo.id)
              }}
        ...
        <div className="w-96 border-l border-terminal-border p-4 overflow-y-auto">
          <SegmentEditor
            projectName={projectName!}
      ...
      {addSegmentAt !== null && addSegmentVideoId && (
        <AddSegmentModal
          isOpen
          onClose={() => {
          ...
          projectName={projectName!}
          videoId={addSegmentVideoId}
          startTime={addSegmentAt}
          videoDuration={project?.videos.find(v => v.id === addSegmentVideoId)?.duration || videoDuration}
          existingSegments={segments.filter(s => s.video_id === addSegmentVideoId)}
        />
      )}
