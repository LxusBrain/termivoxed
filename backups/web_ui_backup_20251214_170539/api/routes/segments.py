...
async def list_segments(
    ...
    Args:
        ...
        all_videos: If True, return segments from ALL videos (for multi-video timeline view)
    """
    ...
    if video_id:
        ...
        # Return segments from ALL videos (for combined multi-video view)
        segments = []
...
async def validate_timeline(project_name: str, video_id: Optional[str] = None):
...
async def create_segment(project_name: str, request: SegmentCreate, video_id: Optional[str] = None):
...
async def get_segment(project_name: str, segment_id: str):
...
async def update_segment(project_name: str, segment_id: str, request: SegmentUpdate):
    """Update a segment"""
    project = Project.load(project_name)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    if target_video.duration and target_segment.end_time > target_video.duration:
...
async def delete_segment(project_name: str, segment_id: str):
    """Delete a segment"""
    project = Project.load(project_name)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
...
async def analyze_segment_timing(project_name: str, segment_id: str):
