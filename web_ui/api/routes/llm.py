...
from web_ui.api.utils.security import validate_video_path
...
async def analyze_video(
    ...
    """
    try:
        # Validate video path for security (prevents path traversal)
...
async def generate_script(
    ...
    try:
        ...
        scripts, warnings = await llm_service.generate_scripts(
            ...
            context_segments=request.context_segments
        )
...
async def refine_script(
    request: ScriptRefinementRequest,
    user: AuthenticatedUser = Depends(get_current_user)
):
...
async def estimate_text_duration(
    request: TextDurationEstimate,
    user: AuthenticatedUser = Depends(get_current_user)
):
...
async def get_available_providers():
    ...
    Returns all supported providers including:
    - Ollama (local)
    ...
    providers = [
        {
            ...
            "configured": health["ollama_available"],
            "models": health["ollama_models"],
            "requires_api_key": False,
            "description": "Run AI models locally on your machine",
            "env_vars": ["OLLAMA_HOST (optional)"],
        },
        {
            "type": "openai",
            "name": "OpenAI",
            "available": True,  # Always available if API key provided
            "configured": health.get("openai_configured", False),
        ...
        },
        {
            ...
            "available": True,
            "configured": health.get("anthropic_configured", False),
            "models": [
                "claude-3-5-sonnet-latest", "claude-3-5-haiku-latest",
                "claude-3-opus-latest", "claude-3-sonnet-20240229", "claude-3-haiku-20240307"
            ],
            "requires_api_key": True,
            "description": "Claude 3.5 and Claude 3 models from Anthropic",
        ...
        },
        {
            ...
            "name": "Azure OpenAI",
            "available": True,
            "configured": health.get("azure_openai_configured", False),
            "models": ["gpt-4o", "gpt-4", "gpt-35-turbo"],
            "requires_api_key": True,
            "description": "OpenAI models on Microsoft Azure",
            "env_vars": ["AZURE_OPENAI_API_KEY", "AZURE_OPENAI_ENDPOINT", "AZURE_OPENAI_DEPLOYMENT_NAME"],
        },
        {
            "type": "google",
            "name": "Google Gemini",
            "available": True,
            "configured": health.get("google_configured", False),
            "models": ["gemini-2.5-flash", "gemini-2.5-pro", "gemini-1.5-flash", "gemini-1.5-pro"],
            "requires_api_key": True,
            "description": "Gemini Pro and Gemini Flash models from Google",
            "env_vars": ["GOOGLE_API_KEY or GEMINI_API_KEY"],
        },
        {
            "type": "aws_bedrock",
            "name": "AWS Bedrock",
            "available": True,
            "configured": health.get("aws_bedrock_configured", False),
            "models": [
        ...
        },
        {
            ...
            "name": "HuggingFace",
            "available": True,
            "configured": health.get("huggingface_configured", False),
            "models": [
            ...
            "env_vars": ["HUGGINGFACEHUB_API_TOKEN"],
        },
        {
            "type": "custom",
            ...
            "description": "Use your own OpenAI-compatible API endpoint (LM Studio, vLLM, etc.)",
            "env_vars": [],
        }
    ]
...
async def intelligent_auto_segment(
    request: IntelligentAutoSegmentRequest,
    user: AuthenticatedUser = Depends(get_current_user)
):
    """
    ...
    """
    try:
        # Validate video path if provided (security: prevents path traversal)
        validated_video_path = None
        if request.video_path:
            validated_video_path = validate_video_path(request.video_path, must_exist=True)
        ...
        if request.stream:
            ...
            return StreamingResponse(
                _stream_intelligent_generation(
                    ...
                    # Video analysis options (use validated path)
