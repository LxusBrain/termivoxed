...
class Segment:
    ...
    # Timing
    start_time: float = 0.0  # seconds - position on timeline
    end_time: float = 0.0    # seconds - position on timeline
    audio_offset: float = 0.0  # seconds - offset into audio file (for start trimming)
    ...
    volume: str = "+0%"
    pitch: str = "+0Hz"
    voice_sample_id: Optional[str] = None  # For voice cloning (Coqui TTS)
    tts_provider: Optional[str] = None  # TTS provider used (edge_tts, coqui, piper, etc.)
    ...
    extends_to_next_video: bool = False  # When True, segment can extend past video duration into next video
    ...
    def validate(self, require_text: bool = False) -> tuple[bool, Optional[str]]:
        """
        Validate segment

        Args:
            require_text: If True, text must not be empty. Default False allows
                         creating/moving segments without text (user can add text later).
