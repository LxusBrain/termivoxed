...
class Video:
    ...
    # Timeline position (for video reordering/repositioning)
    # These are independent of video duration - allow placing video anywhere on timeline
    timeline_start: Optional[float] = None  # Where this clip starts on the timeline
    timeline_end: Optional[float] = None  # Where this clip ends on the timeline

    # Source trimming (which portion of the source video to use)
    # These define the IN and OUT points within the source video file
    source_start: float = 0.0  # Where to start reading from source video (trim start)
    source_end: Optional[float] = None  # Where to stop reading from source video (trim end, None = use full duration)
